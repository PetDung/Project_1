...
public class EditColorFrame extends javax.swing.JFrame {

    ProuductPanel father;
    Color color;
    ColorRepository colorRepository = new ColorRepository();

    public EditColorFrame(ProuductPanel father, Color color) {
        setUndecorated(true);
    ...
    public EditColorFrame(ProuductPanel father) {
        setUndecorated(true);
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        initComponents();
        this.father = father;
    ...
    void fill() {
        ...
        if (color.getColorId() != null) {
            tfName.setText(color.getColorName());
            tfMoTa.setText(color.getDescription());
            lbCapNhat.setText(color.getUpdatedAt().toString());
            lbNgayTao.setText(color.getCreatedAt().toString());
        }

        boolean ia = color.isIsActive() == null ? true : color.isIsActive();
        rdoHoatDong.setSelected(true);
        if (!ia) {
            rdoNgung.setSelected(true);
        }
        color.setIsActive(ia);
    ...
    private void tfMoTaKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tfMoTaKeyReleased
        ...
        color.setDescription(text.trim());
    ...
    private void rdoHoatDongItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_rdoHoatDongItemStateChanged
        color.setIsActive(rdoHoatDong.isSelected());
        System.out.println(color.isIsActive());
    }//GEN-LAST:event_rdoHoatDongItemStateChanged

    private void jButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MouseClicked

        if (color.getColorName() == null || color.getDescription() == null
                || color.getColorName().isBlank() || color.getDescription().isBlank()) {
        ...
        try {
            ...
            father.setListColor(colorRepository.selectAll());
            father.fillTableColor();
            return;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new EditColorFrame().setVisible(true);
            }
        });
    }
